...
echo [93mc / clean - Почистить кэш игры   !beta!
echo [93mreset - [91mсбросить все данные WOTB !beta![0m
...
:: затем вызываем на каждую игру чистильщик кэша
...
:: работаем с "packs" в документах
set "tar=!docs!\packs"
if exist "!tar!" (
    echo.
    echo [94m[ [36mНашёл папку "packs" в документах [94m][0m

    cd /d "!tar!"
    if "%~1"=="entire" (
        echo [36mУдаляю всю папку...[0m
        rd /q /s "!tar!"
    ) else (
        echo [90mПробую удалить данные о последнем сервере игры...[0m
        call :cycle-delete "*footer*" "files"
    )

) else (
    echo [90m[i] Папка "packs" в документах - не найдена[0m
)
